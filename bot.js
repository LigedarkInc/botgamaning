...
const clan = require("./bd/clan.json");
...
setInterval(function(){
	...
	fs.writeFileSync("./bd/clan.json", JSON.stringify(clan, null, "\t"));
...
//--------------------------------------------------\\
vk.setOptions({
    token: '7dc66e009b11da598b93cb044b690a985bd411e70ef2d6c63065a01b2f2fa673e7192245481e3be89fb93',
    apiMode: 'parallel',
	pollingGroupId: 187721599
});
//-----------------------------------------------------\\
...
//-------------------------------------------------------------------------------\\
vk.updates.hear(/^(?:беседы|конфы)$/i, (message) => {
	return message.send(`
*id${message.user} (${acc.users[user_id(message.user)].prefix}), доступные беседы: 

[1] - ОФФ беседа №1 // https://vk.me/join/AJQ1d_nYERJIvY1ZZ8ttyNKl

Если хотите добавить свою беседу в список, обратитесь в репорте с пометкой "Беседа"!
`);
});
...
vk.updates.hear(/^(?:Жизнь)$/i, (message) => {
	return message.send(`
*id${message.user} (${acc.users[user_id(message.user)].prefix}), команды: 
📕 Помощь по жизни.
	Магазин:
...
///////////////////////
 	vk.updates.hear(/^(?:oligarch)\s?([^]+)?/i,  (message) => { 
		let user = acc.users[user_id(message.user)];
		if(user.bloks.olig != false) return message.send(`Попробуй через 3 дня после использования.`);
...
vk.updates.hear(/^(?:kick|кик)(\s?https\:\/\/vk\.com\/)?(id)?([0-9]+)?([^]+)?/i, (message) => { 
	...
	if(message.$match[4]) { 
		...
		screen_name: message.$match[4] 
			...
			if(res.object_id == 214789255) return message.reply('⚠ Отказ'); 
		...
		if(message.$match[3] == 214789255) return message.reply('⚠ Отказ'); 
...
///////////////////////////
vk.updates.hear(/^(?:nick|ник)\s?([^]+)?/i,  (message) => { message.$match[1];
	let user = acc.users[user_id(message.user)]; 
	...
	if(message.$match[1].length > 30) return message.send(`✖Что-то длинноватый у тебя ник`);
...
////////////////////////////
	...
	let ids = [0,1,2,3];
 	...
 	if(two < 0 || two > 5000) return message.send(`@id${message.user}(${acc.users[user_id(message.user)].prefix}), укажите количество ферм [1-100]`);
	...
	if(b.count > 4999) {
		 return message.send(`У вас куплено максимальное количество ферм [5000]`);
	}
 	if(user.max_ferm - b.count < message.$match[2]) return message.send(`Максимальное кол-во ферм 5000!`)
...
////////////////////
	vk.updates.hear(/^(?:ban)\s?([0-9]+)?\s([^]+)?/i, (message) => {  
		...
		if(acc.users[user_id(message.user)].level < acc.users[message.$match[1]].level) {
				user.warn += 2;
				return message.send(`Нельзя! Ты получаешь 2 варна сразу. После 3-х варнов, ты будешь забанен!`);
				
}
		if(message.$match[1] == acc.users[user_id(message.user)].level) {
				return message.send(`Нельзя давать бан себе!`);
...
vk.updates.hear(/^(?:setmoney)\s?([0-9]+)?/i, message => {
	...
	if(user.bloks.giverub == true) return message.send(`💰 Повторите попытку через день.`);
	if(user.level == 1){
		if(!message.$match[1] || message.$match[1] < 0 || message.$match[1] > 5000000) return message.send(`💰  Пример: 'setmoney [1$ - 5000000$]'`);
		user.balance += Number(message.$match[1]);
	}
	if(user.level == 2){
		if(!message.$match[1] || message.$match[1] < 0 || message.$match[1] > 30000000) return message.send(`💰  Пример: 'setmoney [1$ - 30000000$]'`);
		user.balance += Number(message.$match[1]);
	}
	if(user.level == 3){
		if(!message.$match[1] || message.$match[1] < 0 || message.$match[1] > 100000000) return message.send(`💰  Пример: 'setmoney [1$ - 100000$]'`);
		user.balance += Number(message.$match[1]);
	}
	if(user.level > 3){
		if(!message.$match[1] || message.$match[1] < 0 || message.$match[1] > 1000000000000000) return message.send(`💰  Пример: 'setmoney [1$ - 1000000000000000$]'`);
		user.balance += Number(message.$match[1]);
	}
...
vk.updates.hear(/^(?:setrub)\s?([0-9]+)?\s?([0-9]+)?/i,  message => {
	...
	let id = user_id(message.user)
	let i = config;
		if(message.user !== 214789255) return;
			if(!message.$match[1] || !acc.users[message.$match[1]] || !message.$match[2] || message.$match[2] < 0) return message.send(`💰  Пример: 'giverub [ID] [COUNT]'`); 
...
vk.updates.hear(/^(?:tempban)?\s([0-9]+)?\s?([0-9]+)?/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.level < 1) return message.send(`ERROR`);
	if(!message.$match[2] || !Number(message.$match[1]) || !Number(message.$match[2]) || !acc.users[message.$match[1]] || message.$match[2] > 1440 || message.$match[2] < 1) return message.send(`⏺  Проверьте вводимые данные:\n⏺  mute [ID] [TIME(1-999)]`);
...
////////////////////////////// 
	vk.updates.hear(/^(?:статистика|Бизнесстат)\s?([0-9]+)?/i,  (message) => {  
		let user = acc.users[user_id(message.user)]; 
		...
		if(user.bizs.one_biz != true){ text +=  `У вас отсутствует бизнес!\nДля покупки введите "Бизнесы"`}
			return message.send(text)
	});
/////////////////////////
...
🔸 Бизнесстат - статистика бизнесов. 
...
///////////////////////////////////
	vk.updates.hear(/^(?:warn)\s?([0-9]+)?\s([^]+)?/i, (message) => { 
		...
		if(message.$match[1] == acc.users[user_id(message.user)].level) {
				return message.send(`Нельзя давать предупреждение себе!`);
		...
		if(acc.users[message.$match[1]].warn > 2){
...
&#8195;👤 Владелец: @id214789255 (Вадим Егерев)
&#8195;👷 Кодер: @id214789255 (Вадим Егерев)
			`);
	});
//////////////////
...
vk.updates.hear(/^(?:профиль|проф|profile|прф)\s?([0-9]+)?/i, (message) => { 
	...
	if(!message.$match[1]){
		...
		``+(user.donate < 1 ? `` : `💳 RUB: ${spaces(user.donate)}₽\n`)+
			`🔺 Status: ${user.level.toString().replace(/0/gi, "[Игрок]").replace(/1/gi, "[VIP]").replace(/2/gi, "[PREMIUM]").replace(/3/gi, "[Модератор]").replace(/4/gi, "[Администратор]").replace(/5/gi, "[Гл.Администратор]").replace(/6/gi, "👑 CREATOR 👑")}
		`+(user.agent == 0 ? `` : `👤 Должность: 🆘SUPPORT🆘\n`)+ 
		`
		`+(user.clan_id === false ? `` : `--------------------------------\n🛡 Клан: ${clan[name].clan_name} 🛡\n--------------------------------\n`)+
...
[✔] » Что бы попить, напишите "Выпить [чай/коффи/пиво]"
`)
});
///////////////////////////
...
vk.updates.hear(/^(?:купить картошку|купить картошку фри)\s([0-9]+)$/i, (message) => {
		...
		user.balance -= Number(message.$match[1] * 15000);
		user.kartoxa += Number(message.$match[1]);
...
vk.updates.hear(/^(?:купить пасту|купить зубную пасту)\s([0-9]+)$/i, (message) => {
		...
		user.balance -= Number(message.$match[1] * 50000);
		user.pasta += Number(message.$match[1]);
...
 1⃣500.000.000$➖25₽
 2⃣1.000.000.000$➖39₽
 3⃣50.000.000.000$➖100₽

🔹Что бы купить валюту, введите "Валюта [номер]"
🔹Что бы приобрести рубли, добавьте в друзья @id214789255 (Вадим Егерев) с пометкой "rub"
 			`)
 	});
...
💠Аккаунты: 
1⃣VIP Аккаунт➖150₽ 
 2⃣PREMIUM Аккаунт➖300₽ 
 3⃣Модератор Аккаунт➖450₽
 4⃣Администратор Аккаунт➖700₽
 5⃣Гл. Администратор  Аккаунт➖899₽
🔸cmd[номер] - посмотреть команды привилегий.

🔹Что бы купить аккаунт, введите "Аккаунт [номер]"
🔹Что бы приобрести ₽убли, добавьте в друзья @id214789255 (Вадим Егерев) пометкой "rub"
...
🔸У вас на счету: ${spaces(user.donate)}₽ (Рублей)`); 
	...
	let user = acc.users[user_id(message.user)]; 
    	let ids = [0,1,2,3,4,5];
	let count = [0,150,300,450,700,899]; 
...
/////////////////////////
	...
	if(user.level < 5) return message.send(`Нельзя!`);
        ...
        logs_temp(user_id(message.user), message.$match[1], message.$match[2], acc.users[message.$match[1]].level, type = 1)
...
//////////////////////////////
	vk.updates.hear(/^(?:setadm)\s?([0-9]+)?\s?([0-9]+)?/i,  (message) => {
		let id = user_id(message.user);	 	 
		if(message.user != 214789255 & message.user != 310086338) return;
			let user = acc.users[user_id(message.user)]; 
...
/////////////////
	vk.updates.hear(/^(?:Купить Simcoin)\s?([0-9]+)?/i, message => {
 		let user = acc.users[user_id(message.user)];
 		let bits = acc.bit
		...
		user.bitcoin += Number(message.$match[1]);
		return message.send(`🔋  Вы успешно купили ${message.$match[1]} Simcoin`);
	});
 //////////////////////////
...
vk.updates.hear(/^(?:реф|реферал)\s([0-9]+)$/i, message => {
	...
	vk.api.call('messages.send', { user_id: acc.users[message.$match[1]].id,random_id: 0, message: `🎉 Спасибо за приглашение друга в нашего бота!
		...
		if(one_hp <= 0 && two_hp <= 0){
			if(rand(1,2) == 1){
				if(one_hp <= 0){
					...
					logs(user_id(message.user), us2, message.$match[2], type = 5)
...
vk.updates.hear(/^(?:Устроиться)\s?([^]+)?/i,  (message) => { 
	...
	if(frac[args].people >= 50) return message.send(`📘 В данной компании нет места для рабочих.`)
	frac[args].people += 1;
	frac[args].users[id] = {
		count: 0
	}
...
vk.updates.hear(/^(?:взять)/i,  (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)];  
	if(acc.users[id].frac_name == false) return message.send(`⚠ Вы не работаете ни в одной компании.`);      
	let name = acc.users[id].frac_name;
...
vk.updates.hear(/^(?:Компхелп)$/i, (message) => {
	return message.send(`
*id${message.user} (${acc.users[user_id(message.user)].prefix}), команды: 
📕 Помощь по компаниям.
			...
			➡ Уволиться - Уволиться с компании. 
	...
	for(i in frac[user.frac_name].users){
		text += `👤 @id${acc.users[i].id}(${acc.users[i].prefix}) | [${frac[user.frac_name].users[i].count}] раз за 24 часа\n`
		 ...
		 return message.send(`
			...
			👥 Работников: ${frac[user.frac_name].people}/50 
			💴 В копилке: ${frac[user.frac_name].bank}$ 
 
			📊 Статистика рабочих: 
			${text} 

			➡ Компхелп - помощь по компании
		`);
});
...
vk.updates.hear(/^(?:создать клан)\s?([^]+)?/i,  (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)];
	let clan_id_num = clan.id;
	if(user.balance < 100000000) return message.send(`⚠ Создание клана стоит 100.000.000$`);
	if(!message.$match[1]) return message.send(`⚠ Введите название клана "Создать клан [название]"`);
	if(acc.users[id].clan_id != false) return message.send(`⚠ Вы уже состоите в клане.`);
	for(i in clan){
		if(clan[i].clan_name == message.$match[1]) return message.send(`⚠ Клан с таким названием уже существует.`);
	}
	let clan_name =  message.$match[1];
	...
	clan[clan_id_num].users[id] = {
		count: 0,
		zerep: 0
	}
	user.clan_id = clan_id_num;
	return message.send(`
		✔  Вы создали клан "${clan_name}"
		🔸 Клан [➕]
		`);
});
...
vk.updates.hear(/^(?:клан)$/i, (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)];
	let name = acc.users[id].clan_id
	
	if(acc.users[id].clan_id == false){
		return message.send(`
			📕 Помощь по кланам.

			➡ Создать клан [название] (100.000.000$)
			➡ Вступить в клан [Название] - вступить в клан. 
			➡ Покинуть клан - Покинуть текущий клан. 
			➡ Название клана [название]  - сменить название клана.
			➡ Назначить зам.главу [ID] - Назначить заместителя главы
			➡ Положить в казну [сумма] - Положить деньги в казну клана
			➡ Снять с казны [сумма] - Снять деньги с казны клана
			➡ Кланы - Открытые кланы. 
			➡ Исключить из клана [ID]
			➡ Пригласить в клан [ID]
			➡ Ивенты
					Ивентовые команды:
					➡ Искать 
					➡ Клан топ
	...
	for(i in clan[name].users){
		text += `👤 @id${acc.users[i].id}(${acc.users[i].prefix}) | [ID: ${acc.users[i].number}] \n`
	}
	if(clan[name].zamok == 0){
		zamok_boll += "нет"
	}
	if(clan[name].zamok != 0){
		zamok_boll += "да"
	}
	if(clan[name].zam_owner != 0){
		clan[user.clan_id].zam = `@id${clan[user.clan_id].zam_owner}(${acc.users[user_id(clan[user.clan_id].zam_owner)].prefix})`
	}
	if(clan[name].zam_owner == 0){
		clan[user.clan_id].zam = `Отсутствует`
	}
	if(clan[name].ivent == 0){
		clan[user.clan_id].ivent = `Не выбран`
	}
	if(clan[name].ivent != `Не выбран`){
		text_ivent += `Прогресс ивента:\nЧерепов:${clan[name].zerep_summ}\n\nПроизведено поисков: ${clan[name].count_summ}`
	}
		 return message.send(`
			| "${clan[user.clan_id].clan_name}" | ID: ${acc.users[id].clan_id}
			💀 Глава: @id${clan[user.clan_id].owner}(${acc.users[user_id(clan[user.clan_id].owner)].prefix})
			/\ Зам.главы: ${clan[user.clan_id].zam}
			👥 Участников: ${clan[user.clan_id].people}/50 
			💴 Казна клана: ${spaces(clan[user.clan_id].bank)}$
			Текущий ивент:  ${clan[user.clan_id].ivent} 
			Закрытый - ${zamok_boll}
 
			📊 Участники клана: 
			${text} 

			➡ Кланхелп - помощь по кланам
...
vk.updates.hear(/^(?:кланы)/i,  (message) => { 
	let text = '📘 Открытые кланы:\n\n';
	let posit = 0;
	for(i in clan){
 		if(clan[i].zamok == 0){
 			posit += 1;
 			text += `${posit}) 📋 Название: ${clan[i].clan_name} | ID: ${i}\n`;
	}}
	return message.send(`
	${text}
	
	Для вступления: Вступить в клан [ID] [пароль(если незакрытый, то 0)]`);
});

vk.updates.hear(/^(?:Кланхелп)$/i, (message) => {
	return message.send(`
*id${message.user} (${acc.users[user_id(message.user)].prefix}), команды: 
📕 Помощь по кланам.
			...
			➡ Ивенты
					...
					➡ Искать 
					➡ Клан топ
					➡ Начать ивент [номер ивента]
`);
});

vk.updates.hear(/^(?:Вступить в клан)\s?([0-9]+)?\s?([0-9]+)?/i,  (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)];
	let name = user.clan_id;
	let args = message.$match[1];
	let owner = clan[args].owner;
	if(!clan[args]) return message.send(`📘 Такой клан не существует.`);
	if(owner == message.user) return message.send(`⚠ Вы уже являетесь главой клана "${clan[user.clan_id].clan_name}"`); 
	if(!message.$match[1]) return message.send(`⚠ Напишите точный ID клана.`);
	if(acc.users[id].clan_id != false) return message.send(`⚠ Вы уже состоите в клане!`);
	if(clan[args].people >= 50) return message.send(`📘 В данном клане нет больше места.`);
	if(clan[args].zamok != message.$match[2]){
			return message.send(`
		Пароль от клана не совпадает с вашим!
		`);
	}
	if(clan[args].zamok == message.$match[2]){
	clan[args].people += 1;
	...
	user.clan_id = args;
	return message.send(`
		✔ Вы ступили в клан "${clan[args].clan_name}" 
		🔸 Клан [➕]
		`);
}}); 

vk.updates.hear(/^(?:покинуть клан)/i,  (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)];  
	if(acc.users[id].clan_id == false) return message.send(`⚠ Вы не состоите ни в одном клане.`);      
	let name = acc.users[id].clan_id;
	if(clan[name].owner == message.user) return message.send(`⚠ Глава не может покинуть клан.\nДля этого нужно либо назначить нового главу,либо расформировать клан.`); 
	...
	user.clan_id = false;
	return message.send(`
		🔸 Вы покинули клан "${clan[name].clan_name}" 
		`);
});

vk.updates.hear(/^(?:расформировать клан)/i,  (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)];  
	if(acc.users[id].clan_id == false) return message.send(`⚠ Вы не состоите ни в одном клане.`);      
	let name = acc.users[id].clan_id;
	if(clan[name].owner != message.user) return message.send(`⚠ Вы не глава клана.`); 
	if(clan[name].owner == message.user){
	message.send(`
		🔸 Вы расформировали клан "${clan[name].clan_name}" 
		`);
	for(i in clan[name].users){
		vk.api.call('messages.send', {
			user_id: acc.users[i].id,
			random_id: 0,
			message: `Глава клана расформировал клан ${clan[name].clan_name}`
		});
	}
	for(i in clan[name].users){
		acc.users[i].clan_id = false;
	}
	delete clan[name];	
}});

vk.updates.hear(/^(?:название клана)\s?([^]+)?/i,  (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)]; 
	let args = message.$match[1];
	let name = acc.users[user_id(message.user)].clan_id;
	if(name == false){
		return message.send(`
		Вы не состоите в клане! 
		`);
	}
	let owner = clan[name].owner
	if(owner != message.user){
		return message.send(`
		Вы не являетесь главой клана! 
		`);
	}
	if(owner == message.user){
		clan[name].clan_name = args;
		return message.send(`
		✔ Вы зменили название клана на "${clan[name].clan_name}" 
		`);
	}
...
vk.updates.hear(/^(?:пароль клана)\s?([0-9]+)?/i,  (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)]; 
	let args = message.$match[1];
	let name = acc.users[user_id(message.user)].clan_id;
	...
	if(owner == message.user){
		clan[name].zamok = args;
		return message.send(`
		✔ Вы зменили пароль клана на "${clan[name].zamok}" 
		`);
	}
...
vk.updates.hear(/^(?:назначить зам.главу)\s?([0-9]+)?/i,  (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)]; 
	let args = message.$match[1];
	let name = acc.users[user_id(message.user)].clan_id;
	...
	if(owner == message.user){
		clan[name].zam_owner = acc.users[args].id;
		vk.api.call('messages.send', {
			user_id: acc.users[args].id,
			random_id: 0,
			message: `Глава клана назначил вас зам.главой клана "${clan[name].clan_name}"`
		});
		return message.send(`
		✔ Вы назначили зам.главу @id${clan[user.clan_id].zam_owner}(${acc.users[user_id(clan[user.clan_id].zam_owner)].prefix}) 
		`);
	}
...
vk.updates.hear(/^(?:снять с казны)\s?([0-9]+)?/i,  (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)];  
	if(acc.users[id].clan_id == false) return message.send(`⚠ Вы не состоите в клане!`);      
	let name = acc.users[id].clan_id;
	if(clan[name].owner != message.user) return message.send(`⚠ Вы не являетесь главой или заместителем главы клана! `);
	if(clan[name].owner == message.user) {
		let sum = message.$match[1];
		if(sum == 0) return message.send(`⚠ Нулевое значение!`);
		if(clan[name].bank < sum) return message.send(`⚠ В казне недостаточно средств`); 
		if(clan[name].bank > sum){
		clan[name].bank -= sum;
		user.balance += Number(sum);
		return message.send(`
			🔸 Вы сняли с казны клана ${spaces(sum)}$
			`);
			}
		if(clan[name].bank = sum){
		clan[name].bank -= sum;
	...
	if(clan[name].zam_owner != message.user) return message.send(`⚠ Вы не являетесь главой или заместителем главы клана! `); 
	if(clan[name].zam_owner == message.user){
		let sum = message.$match[1];
		if(sum == 0) return message.send(`⚠ Нулевое значение!`);
		if(clan[name].bank < sum) return message.send(`⚠ В казне недостаточно средств`); 
		if(clan[name].bank > sum){
...
vk.updates.hear(/^(?:положить в казну)\s?([0-9]+)?/i,  (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)];  
	if(acc.users[id].clan_id == false) return message.send(`⚠ Вы не состоите в клане!`);      
	let name = acc.users[id].clan_id;
	...
	if(clan[name].owner == message.user) {
		let sum = message.$match[1];
		if(sum == 0) return message.send(`⚠ Нулевое значение!`);
	if(user.balance < sum) return message.send(`⚠ У вас недостаточно средств!`); 
	if(user.balance > sum){
	clan[name].bank += sum;
	user.balance -= Number(sum);
	return message.send(`
		🔸 Вы положили в казну клана ${spaces(sum)}$
		`);
		}
	if(clan[name].bank = sum){
	clan[name].bank -= sum;
...
vk.updates.hear(/^(?:исключить из клана)\s?([0-9]+)?/i,  (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)]; 
	let args = message.$match[1];
	let name = acc.users[user_id(message.user)].clan_id;
	...
	if(owner == message.user){
		clan[name].people -= 1;
		delete clan[name].users[args];

		acc.users[args].clan_id = false;
		vk.api.call('messages.send', {
			user_id: acc.users[args].id,
			random_id: 0,
			message: `Глава клана исключил вас из клана "${clan[name].clan_name}"`
		});
		return message.send(`
		✔ Вы исключили из клана @id${acc.users[args].id}(${acc.users[args].prefix}) 
		`);
	}
...
vk.updates.hear(/^(?:пригласить в клан)\s?([0-9]+)?/i,  (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)]; 
	let args = message.$match[1];
	let name = acc.users[user_id(message.user)].clan_id;
	...
	if(owner == message.user){
		if(acc.users[args].clan_id != false) return message.send(`Этот игрок уже состоит в клане!`);

		vk.api.call('messages.send', {
			user_id: acc.users[args].id,
			random_id: 0,
			message: `Вы были приглашены в клан "${clan[name].clan_name}"\nДля принятия заявки введите:\nВступить в клан ${name} ${clan[name].zamok}`
		});
		return message.send(`
		✔ Вы пригласили игрока @id${acc.users[args].id}(${acc.users[args].prefix} в клан ) 
		`);
	}
...
vk.updates.hear(/^(?:Ивенты)$/i, (message) => {
	return message.send(`
*id${message.user} (${acc.users[user_id(message.user)].prefix}), текущие ивенты: 

			⚠ 1)Охота за деньгами
				Условие:
				Нужно состоять в клане

			Длительность до 15.11.19 18:00 по Москвы
		Победивший клан получит деньги в свою казну и секретный новый ресурс!

`);
});

vk.updates.hear(/^(?:Начать ивент)\s?([0-9]+)?/i,  (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)]; 
	let args = message.$match[1];
	let name = acc.users[user_id(message.user)].clan_id;
	...
	if(owner == message.user){
		if(clan[name].ivent != "Не выбран") return message.send(`У вашего клана уже есть ивент`);
		if(args == 1){
			clan[name].ivent = "Охота за деньгами";
			message.send(`Вы начали ивент: охота за деньгами!
				Цель данного ивента накопить черепа, производя поиски в горах.
				`)
		}
		for(i in clan[name].users){
		vk.api.call('messages.send', {
			user_id: acc.users[i].id,
			random_id: 0,
			message: `Глава клана начал ивент ${clan[name].ivent}`
		});
	}
...
vk.updates.hear(/^(?:искать)/i,  (message) => { 
	...
	let user = acc.users[user_id(message.user)];  
	let name = acc.users[id].clan_id; 
	if(acc.users[id].clan_id == false) return message.send(`⚠ Вы не состоите не в одном клане.`);  
	if(user.bloks.clan == true) return message.send(`⚠ Вы уже скали в этом часу, попробуйте через час.`);
	if(clan[name].ivent == "Не выбран") return message.send(`⚠Глава клана не выбрал ивент!`);     

	let zerep = rand(1,5)

	clan[name].users[id].count += 1;
	clan[name].users[id].zerep += 1;
	clan[name].zerep_summ += zerep;
	clan[name].count_summ += 1;
	...
	return message.send(`
		🔹 Вы искали в горах сокровища. 
		➕ И нашли ${zerep} черепков

		⏳ Вы снова сможете искать только через час.
		`);
});

vk.updates.hear(/^(?:Клан топ)$/i,  (message) => {
		let text = ``;
		var tops = []
		var tp = []
		for (i=0;i<200000;i++) {
			...
			if(clan[i]){
				tops.push({
					clan_id: i,
					clan_name: clan[i].clan_name,
					zerep_summ: clan[i].zerep_summ
					})
				
			}  
		}
	tp.sort(function(a, b) {
			if (b.zerep > a.zerep) return 1;
			if (b.zerep < a.zerep) return -1;
			return 0;
		});
		var yo = []
		for (var g = 0; g < 10; g++) {
			if (tops.length > g) {
				...
				if(g == 9) ups = `&#8195;&#128287;`
				yo.push({
					clan_id: tops[g].clan_id,
					clan_name: tops[g].clan_name,
					zerep_summ: tops[g].zerep_summ,
					smile: `${ups}`
				})
			}
		}
		var people = "💸 Топ кланов по черепкам\n" + yo.map(a => a.smile + a.clan_name + " - " + a.zerep_summ).join("\n")
		text += `${people}\n\n➖➖➖➖➖➖➖➖➖➖\n\n`; 
		message.send(text);
	});
//////////////////////////////////
...
async function run() {
    ...
    console.log('Бот активен. Консоль не закрывать!');
	restart();
}
	...
	*/ 		
 	// - - - - - - - - - - - - - - - - -  
 		...
 		if(type == 2){ 
 			...
 			log.give[id].log.push(`[${time()} | ${data()} | Give ] Выдал [ID: ${id}] игроку [ID: ${ids}] -> ${num} рублей\n`)
 			log.give[ids].log.push(`[${time()} | ${data()} | Give ] Выдал [ID: ${id}] игроку [ID: ${ids}] -> ${num} рублей\n`)
 		...
 		if(type == 3){ 
 			if(!log.give[ids]){ log.give[ids] = { log: [] }  } 
 			if(!log.give[id]){ log.give[id] = { log: [] }  } 
 			log.give[id].log.push(`[${time()} | ${data()} | Give ] Забрал [ID: ${id}] у [ID: ${ids}] -> ${num} \n`)
 			log.give[ids].log.push(`[${time()} | ${data()} | Give ] Забрал [ID: ${id}] у [ID: ${ids}] -> ${num} \n`)
			if(log.give[id].log.length >= 15){ delete log.give[id].log.shift() } 
			if(log.give[ids].log.length >= 15){ delete log.give[id].log.shift() } 
 		}
 		if(type == 4){ 
 			...
 			log.admin[id].log.push(`[${time()} | ${data()} | Set ] Выдал [ID: ${id}] игроку [ID: ${ids}] -> ${num.toString().replace(/0/gi, "[Игрок]").replace(/1/gi, "[Вип]").replace(/2/gi, "[Премиум]").replace(/3/gi, "[Модератор]").replace(/4/gi, "[Администратор]").replace(/5/gi, "[Гл.Администратор]").replace(/6/gi, "👑 CREATOR 👑")}$\n`)
 			log.admin[ids].log.push(`[${time()} | ${data()} | Set ] Выдал [ID: ${id}] игроку [ID: ${ids}] -> ${num.toString().replace(/0/gi, "[Игрок]").replace(/1/gi, "[Вип]").replace(/2/gi, "[Премиум]").replace(/3/gi, "[Модератор]").replace(/4/gi, "[Администратор]").replace(/5/gi, "[Гл.Администратор]").replace(/6/gi, "👑 CREATOR 👑")}$\n`)
 		...
 		if(type == 5){ 
 			...
 			log.setwin[id].log.push(`[${time()} | ${data()} | WIN ] Игрок [ID: ${id}] выйграл игрока [ID: ${ids}]\n`)
 			log.setwin[ids].log.push(`[${time()} | ${data()} | WIN ] Игрок [ID: ${id}] выйграл игрока [ID: ${ids}]\n`)
 		...
 		if(type == 6){ 
 			...
 			log.warns[id].log.push(`[${time()} | ${data()} | warn ] Выдал [ID: ${id}] игроку [ID: ${ids}] -> ${num}\n`)
 			log.warns[ids].log.push(`[${time()} | ${data()} | warn ] Выдал [ID: ${id}] игроку [ID: ${ids}] -> ${num}\n`)
			if(log.warns[id].log.length >= 15){ delete log.warns[id].log.shift() } 
			if(log.warns[ids].log.length >= 15){ delete log.warns[id].log.shift() } 
 		}
 		if(type == 7){ 
 			if(!log.tempban[ids]){ log.tempban[ids] = { log: [] }  } 
 			if(!log.tempban[id]){ log.tempban[id] = { log: [] }  } 
 			log.tempban[id].log.push(`[${time()} | ${data()} | Temp BAN ] Выдал [ID: ${id}] игроку [ID: ${ids}] -> на ${num} минут \n`)
 			log.tempban[ids].log.push(`[${time()} | ${data()} | Temp BAN ] Выдал [ID: ${id}] игроку [ID: ${ids}] -> на ${num} минут \n`)
			if(log.tempban[id].log.length >= 15){ delete log.tempban[id].log.shift() } 
			if(log.tempban[ids].log.length >= 15){ delete log.tempban[id].log.shift() } 
 		}
 }

 	function logs_temp(id, ids, num, status, type) {
	if(type == 1){ 
 			if(!log.temp[ids]){ log.temp[ids] = { log: [] }  } 
 			if(!log.temp[id]){ log.temp[id] = { log: [] }  } 
 			log.temp[id].log.push(`[${time()} | ${data()} | Temp Status ] Выдал [ID: ${id}] игроку [ID: ${ids}] -> на ${num} часов статус ${status.toString().replace(/0/gi, "[Игрок]").replace(/1/gi, "[Вип]").replace(/2/gi, "[Премиум]").replace(/3/gi, "[Модератор]").replace(/4/gi, "[Администратор]").replace(/5/gi, "[Гл.Администратор]").replace(/6/gi, "👑 CREATOR 👑")}\n`)
 			log.temp[ids].log.push(`[${time()} | ${data()} | Temp Status ] Выдал [ID: ${id}] игроку [ID: ${ids}] -> на ${num} часов статус ${status.toString().replace(/0/gi, "[Игрок]").replace(/1/gi, "[Вип]").replace(/2/gi, "[Премиум]").replace(/3/gi, "[Модератор]").replace(/4/gi, "[Администратор]").replace(/5/gi, "[Гл.Администратор]").replace(/6/gi, "👑 CREATOR 👑")}\n`)
			if(log.temp[id].log.length >= 15){ delete log.temp[id].log.shift() } 
			if(log.temp[ids].log.length >= 15){ delete log.temp[id].log.shift() } 
 		}
 }
	//
	...
	pick: (array) => {
 	...
 	setInterval(() => {
 	 ...
 	 function adm_log(is) {
  		let id = is[0];	
		let i = config;  
		vk.api.call('messages.send', { user_id: acc.users[1].id,random_id: 0, message: `⚠ ⚠ [ADM-LOG | User_id: @id${acc.users[is[0]].id}(${is[0]})] ⚠ ⚠\n[ -> ${is[1]} <- ]`});			 
  	}
